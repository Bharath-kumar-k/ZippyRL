...
train_file_handler = logging.FileHandler('training.log', mode='a')
...
def train_agent(env, agent, episodes, batch_size, verbose=False, fileName='', load_file_number_train=0):
    jerkActionMap = [env.jMax, 0, -env.jMax]
    ...
    if load_file_number_train != 0:
        agent.load(fileName+'/saved_weights'+str(load_file_number_train))
        train_file_handler.mode = 'a'
    ...
    for episode in range(episodes):
            ...
            if done:
                if verbose:
                    ...
                    print("Time: {}, Distance to goal: {}, Velocity: {}, Acceleration: {}, Jerk Input: {}".format(
                        ...
                        jerkActionMap[action]))
                # print("Episode {}/{} completed | Episode Reward: {}".format(episode + 1, episodes, episode_reward))
                train_logger.info(f'Episode {episode + 1 + load_file_number_train}/{episodes} completed | Episode Reward: {episode_reward}')
        ...
        if (episode + 1) % log_frequency == 0:
            ...
            agent.save(name=fileName+'/saved_weights'+str(episode + 1 + load_file_number_train))
            train_logger.info(f'Average Reward (Episodes {episode - log_frequency + 2 + load_file_number_train}-{episode + 1 + load_file_number_train}): {avg_reward}')
            print("--------------------------------------------- Model Saved --------------------------------------------- ")
